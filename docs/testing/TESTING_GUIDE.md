...
2. [Docker Окружение](#docker-окружение)
3. [Изоляция Категорий](#изоляция-категорий)
...
## Docker Окружение
Тесты Cursor (`--cursor`) автоматически проверяют статус контейнера `cursor-agent`:
- **Перед тестом**: Проверка доступности Docker и запуск контейнера.
- **Во время**: Контейнер остается активным для ускорения последующих тестов.
- **При ошибке**: Автоматический вывод последних 30 строк логов контейнера.

## Изоляция Категорий
Тесты строго разделены по флагам для предотвращения дублирования:
- `--openrouter`: Тесты API и обнаружения моделей.
- `--api`: Тесты HTTP-сервера.
- `--cursor`: Интеграция с Cursor IDE (требует Docker).
- `--llm`: Базовая логика `LLMManager`.
- `--validation`: Проверка конфигураций и безопасности.
- `--checkpoint`: Сохранение и восстановление состояния.
- `--full`: Полный цикл работы агента.
...
1. Проверьте логи: `docker logs cursor-agent`
2. Проверьте статус: `docker inspect cursor-agent`
3. Перезапустите контейнер: `docker restart cursor-agent`
...
- [test/README.md](../../test/README.md) — Информация о тестах в папке test/
- [Advanced Configuration](../guides/advanced_configuration.md) — Настройка API ключей и Git
