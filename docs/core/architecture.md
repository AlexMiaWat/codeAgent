...
### 1. Принцип изоляции проекта (Project Context Enforcement)

Это фундаментальное и неизменное правило системы: **Все CLI контейнеры и агенты должны работать исключительно в пространстве целевого проекта (`PROJECT_DIR`), а не в директории самого Code Agent.**

- **Целевой проект (`PROJECT_DIR`)**: Директория, где находятся исходный код, `todo.md` и документация проекта, над которым работает агент.
- **Директория агента (`codeAgent/`)**: Содержит только код самого агента, его конфигурацию и файлы сессий/чекпоинтов.
- **Docker-контейнеры**: Целевой проект монтируется в контейнер по пути `/workspace`. Все операции внутри контейнера выполняются строго в этой директории.
- **Исключения**: Только файлы сессий (`.session`) и чекпоинтов (`.checkpoint`) хранятся в `codeAgent/` для обеспечения возможности восстановления независимо от состояния целевого проекта.

### 2. Принцип слабой модели (Weak LLM Principle)
...
Определения агентов для выполнения различных типов задач. Подробнее об управлении моделями см. в [LLM Management](llm_management.md).
...
Подробнее см. [Интеграция с Cursor](../integration/cursor_integration.md) и [Logging](logging.md).
...
**Доступ к туду листам других проектов:** Сервер не запрещает чтение туду листов из других проектов, но работает только с одним целевым проектом, указанным в `project.base_dir`. Это позволяет использовать общие туду листы или ссылки на задачи в других репозиториях, если это необходимо.
