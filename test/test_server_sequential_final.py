    ...
    try:
        ...
        def start_server_2():
            ...
            if process1.poll() is not None:
                ...
                print("  [Поток 2] Сервер 1 еще работает")
            if process2.poll() is not None:
                print(f"  [Поток 2] Сервер 2 завершился (код: {process2.returncode})")
            else:
                print("  [Поток 2] Сервер 2 работает")
        ...
        if len(running_processes) <= 1:
            ...
            print("  Это означает, что система корректно завершает старые процессы")
        else:
            print(f"\n[WARN] Работает {len(running_processes)} серверов одновременно")
            print("  Это может быть нормально, если серверы еще не успели завершиться")
