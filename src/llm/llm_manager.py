...
class LLMManager:
    ...
    def __init__(self, config_path: str = "config/llm_settings.yaml", skip_llm_checks: bool = False):
        """
    ...
    def get_fastest_model(self) -> Optional[ModelConfig]:
        ...
        if (self._fastest_model_cache is not None and current_time - self._cache_timestamp < self._cache_ttl):
            return self._fastest_model_cache
        
        primary = self.get_primary_models()
        if not primary: return None
        
        sorted_models = sorted(primary, key=lambda m: m.last_response_time if m.last_response_time > 0 else 0.0)
    ...
    async def analyze_report_and_decide(self, report_content: str, report_file: str, next_instruction_name: str, task_id: str) -> Dict[str, Any]:
        """Анализирует репорт и принимает решение"""
        if self.skip_llm_checks:
            return {"decision": "continue", "reason": "Skipped checks", "next_instruction_name": next_instruction_name, "free_instruction_text": ""}
        ...
        response = await self.generate_response(prompt, response_format={"type": "json_object"}, use_fastest=True)
        if not response.success:
            return {"decision": "continue", "reason": "Error in analysis", "next_instruction_name": next_instruction_name, "free_instruction_text": ""}
            
        try:
            import json
            return json.loads(response.content)
        except:
            return {"decision": "continue", "reason": "JSON parse error", "next_instruction_name": next_instruction_name, "free_instruction_text": ""}

    async def analyze_decision_response(self, decision_data: Dict[str, Any], original_report_file: str, task_id: str) -> Dict[str, Any]:
        ...
        if decision == "continue":
            return {"action": "continue", "next_instruction_name": next_instruction_name, "free_instruction_text": "", "reason": reason}
        elif decision == "insert_instruction":
            return {"action": "execute_free_instruction", "next_instruction_name": next_instruction_name, "free_instruction_text": free_instruction_text, "reason": reason}
        else:
            return {"action": "stop_and_check", "next_instruction_name": next_instruction_name, "free_instruction_text": "", "reason": reason}
