...
class CodeAgentServer:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if GEMINI_AVAILABLE:
            ...
            logger.info(f"Creating Gemini CLI interface with container_name={cli_config.get('container_name')}, timeout={cli_timeout}")
            self.gemini_cli = create_gemini_cli_interface(
                project_dir=str(self.project_dir),
                timeout=cli_timeout,
                container_name=cli_config.get('container_name')
            )
            logger.info(f"Gemini CLI created: {self.gemini_cli}, use_docker={self.gemini_cli.use_docker if self.gemini_cli else None}")
            self.use_gemini_cli = (
                self.gemini_cli and
...
{{
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if "timeout" in error_message.lower() or "timeout" in stderr.lower():
                    ...
                    logger.warning(f"🔌 Agent CLI недоступен при выполнении инструкции {instruction_num}, повторяем...")
                    should_retry = True
    ...
    def _handle_cursor_error(self, error_message: str, task_logger: TaskLogger) -> bool:
        ...
        logger.info(f"Обработка ошибки Agent CLI: error_message='{error_message}', is_critical={is_critical}, is_unexpected={is_unexpected}")
        ...
        with self._cursor_error_lock:
            ...
            if is_critical:
                ...
                logger.error("---")
                task_logger.log_error(f"Критическая Ошибка Agent CLI (не исправится): {error_message}", Exception(error_message))
            ...
            if is_unexpected and self._cursor_error_count <= 2:
                ...
                if self.cursor_cli and hasattr(self.cursor_cli, 'cli_command'):
                    ...
                    if self.cursor_cli.cli_command == "docker-compose-agent":
                        logger.warning(f"Непредвиденная Ошибка Agent CLI (#{self._cursor_error_count}): {error_message}")
                        logger.warning("Перезапускаем Docker контейнер из-за непредвиденной ошибки...")
                        task_logger.log_warning(f"Непредвиденная Ошибка Agent CLI - перезапуск Docker: {error_message}")
                    ...
                    logger.warning("Agent CLI недоступен или не имеет cli_command, пропускаем перезапуск для непредвиденной ошибки")
            elif is_unexpected:
            ...
            task_logger.log_warning(f"Ошибка Agent CLI #{self._cursor_error_count}, задержка перед следующим запросом: {self._cursor_error_delay}с")
        ...
        try:
            ...
            while time.time() - start_time < timeout:
                ...
                if self._check_reload_needed():
                    logger.warning(f"Обнаружено изменение кода во время ожидания файла результата - продолжаем ожидание")
  ...
  "comment": "краткий комментарий о полезности задачи"
            ...
            if not response.success:
                if content:
                    ...
                    logger.warning(
                        f"│ ❌ Не удалось выполнить проверку полезности для задачи"
                    )
                    logger.info(Colors.colorize(
                        f"└───────────────────────────────────────────────────────────",
                        Colors.BRIGHT_CYAN
...
Если пункт туду НЕ соответствует ни одному пункту плана, верни matches=false с причиной."""
            ...
            if not response.success:
                if content:
                    ...
                    logger.warning(f"Не удалось выполнить проверку соответствия для задачи {task_id}")
    ...
    async def _execute_free_instruction(
        ...
        try:
            ...
            logger.info(Colors.colorize("✅ Инструкция принята Cli agent, ожидаем результат...", Colors.GREEN))
    ...
    async def _execute_task(self, todo_item: TodoItem, task_number: int = 1, total_tasks: int = 1) -> bool:
        ...
        if completion_info["has_partial_completion"]:
            ...
            if decision == "postpone_task":
                logger.info(Colors.colorize(f"🤖 По решению LLM Manager задача '{todo_item.text[:50]}...' отложена до конца списка TODO", Colors.BRIGHT_MAGENTA))
    ...
    async def _execute_task_via_cursor(self, todo_item: TodoItem, task_type: str, task_logger: TaskLogger) -> bool:
        ...
        if instruction_progress and instruction_progress.get("last_completed_instruction", 0) > 0:
            ...
            if total_saved == len(all_templates):
                ...
                logger.info(Colors.colorize(f"🤖 LLM Manager решил: {decision_type} - {reason}", Colors.BRIGHT_MAGENTA))
        ...
        for instruction_num, template in enumerate(all_templates, start=1):
            ...
            if self._check_reload_needed():
                logger.warning(f"Обнаружено изменение кода во время выполнения задачи {task_id} - пропускаем инструкцию и продолжаем")
                task_logger.log_warning("Обнаружено изменение кода - пропускаем инструкцию и продолжаем")
            ...
            if not result.get("success"):
                ...
                if can_continue:
                    try:
                        ...
                        """
                        response = await llm_manager.generate_response(prompt=prompt, use_fastest=True)
                        logger.info(f"🤖 LLM Manager анализ ошибки инструкции {instruction_num}: {response.content[:200]}")
                ...
                with self._stop_lock:
                    if self._should_stop:
                        ...
                        logger.error("---")
                        task_logger.log_error("Критическая Ошибка Agent CLI - выполнение задачи прервано", Exception(error_message))
                        self.status_manager.update_task_status(
                            task_name=todo_item.text,
                            status="Прервано",
                ...
                if fallback_used or primary_model_failed:
                    ...
                    if billing_fallback_used:
                        ...
                        logger.info(f"ℹ️ Выполнено с использованием резервной модели - это штатная ситуация для обеспечения надежности")
                        logger.debug(f"Основная модель испытала проблемы, но задача выполнена успешно с резервной моделью")
                else:
            ...
            if self._check_reload_needed():
                ...
                task_logger.log_warning("Обнаружено изменение кода - пропускаем ожидание результата и продолжаем")
                ...
                for _ in range(self.task_delay):
                    with self._stop_lock:
                        if self._should_stop:
                            break
                    if self._check_reload_needed():
                        logger.warning("Обнаружено изменение кода во время задержки между задачами - продолжаем без задержки")
        ...
        with self._reload_lock:
            ...
            if self._reload_after_instruction:
                ...
                if self._restart_count >= 7:
                    logger.warning("Достигнуто максимальное количество перезапусков (7) - выполняется остановка сервера")
                    ...
                    logger.warning(f"Остановка сервера инициирована после достижения лимита перезапусков. Финальный счетчик: {self._restart_count}")
