            ...
            if last_completed <= len(all_templates):
                ...
                if check_report and last_completed < len(all_templates):  # Не проверяем после последней инструкции
                    ...
                    if action == 'execute_free_instruction':
                        ...
                        # Останавливаем весь процесс по решению системы
                        logger.warning(f"🛑 Процесс остановлен по результатам проверки репорта инструкции {last_completed}: {reason}")
                        task_logger.log_warning(f"Процесс остановлен по результатам проверки репорта: {reason}")

                        self.status_manager.update_task_status(
                            task_name=todo_item.text,
                            status="Прервано",
                            details=f"Остановлено по результатам проверки репорта инструкции {last_completed}: {reason}"
                        )

                        # Помечаем задачу как завершенную с ошибкой
                        self.checkpoint_manager.mark_task_completed(task_id, success=False, error_message=reason)
                        return False  # Выходим из функции, задача остановлена
